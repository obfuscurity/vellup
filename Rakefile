...
namespace :db do
  ...
  namespace :migrate do
    ...
    DB = Sequel.connect(ENV['HEROKU_SHARED_POSTGRESQL_URL'])

    desc "Perform migration reset (full erase and migration up)"
    task :reset do
      Sequel::Migrator.run(DB, "models/migrations", :target => 0)
    ...
    task :to do
      ...
      Sequel::Migrator.run(DB, "models/migrations", :target => version)
